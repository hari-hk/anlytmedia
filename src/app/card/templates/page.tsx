...
const SocialIcon = ({ type, url, className = 'w-4 h-4' }) => {
  if (!url) return null;
  const icons = {
    instagram: Instagram,
    facebook: Facebook,
...
const ContactRow = ({ icon: Icon, text, className = '' }) => (
...
const CardTemplate = ({ variant, data }) => {
  const {
    name,
    position,
    org,
  ...
  const baseCard =
    'w-full aspect-[1.7/1] relative overflow-hidden rounded-xl transition-all duration-300 hover:scale-[1.02]';
      ...
      <div className='max-w-7xl mx-auto'>
        ...
        <div className='mb-12 text-center'>
          ...
          <p className='text-slate-500 max-w-2xl mx-auto'>
            Viewing 25 unique templates generated from JSON data. Compatible
